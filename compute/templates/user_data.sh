...
if [[ "$HOSTNAME" =~ "1-server" ]]; then
    ...
    curl -sfL https://get.k3s.io | K3S_TOKEN="${cluster_token}" sh -s - server --cluster-init --vpn-auth="name=tailscale,joinKey=${tailscale_auth_key}" --write-kubeconfig-mode=644

elif [[ "$HOSTNAME" =~ "server" ]]; then
    echo "[user_data.sh] Installing k3s server..."
    curl -sfL https://get.k3s.io | K3S_TOKEN="${cluster_token}" sh -s - server --server="https://${k3s_api}:6443" --vpn-auth="name=tailscale,joinKey=${tailscale_auth_key}" --write-kubeconfig-mode=644

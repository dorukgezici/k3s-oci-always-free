...
if [[ "$HOSTNAME" =~ "1-server" ]]; then
    ...
    curl -sfL https://get.k3s.io | K3S_TOKEN="${cluster_token}" sh -s - server --server="https://${k3s_api_domain}:6443" --tls-san="${k3s_api_domain}" --vpn-auth="name=tailscale,joinKey=${tailscale_auth_key}" --write-kubeconfig-mode=644

else
    echo "[user_data.sh] Installing k3s agent..."
    curl -sfL https://get.k3s.io | K3S_TOKEN="${cluster_token}" sh -s - agent --server="https://${k3s_api_domain}:6443" --vpn-auth="name=tailscale,joinKey=${tailscale_auth_key}"
fi
